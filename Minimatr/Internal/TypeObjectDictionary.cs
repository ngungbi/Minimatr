...
public class TypeObjectDictionary<TObject> : IDictionary<Type, TObject> {
    // private TObject[] _values = Array.Empty<TObject>();
    ...
    private int IndexOf(int hash) {
        ...
        var hi = _count - 1;
        while (lo <= hi) {
            var mid = (hi + lo) / 2; // lo + (hi - lo) / 2;
            var midHash = _hashes[mid];
            if (midHash == hash) return mid;
    ...
    private int FindSpot(int hash) {
        ...
        // if (_count == _hashes.Length) return _hashes.Length;
        var lo = 0;
    ...
    public bool TryAdd(KeyValuePair<Type, TObject> item) {
        ...
        if (IndexOf(hash) != -1) return false;
        var spot = FindSpot(hash);
        var length = _hashes.Length;
        if (length == _count) Expand();

        if (spot < length) AddSpaceAt(spot);
    ...
    IEnumerator<KeyValuePair<Type, TObject>> IEnumerable<KeyValuePair<Type, TObject>>.GetEnumerator() => GetEnumerator();
    ...
    public bool Remove(KeyValuePair<Type, TObject> item) {
        ...
        var index = IndexOf(key);
        if (index == -1) return false;
        if (!_storage[index].Equals(item)) return false;
        RemoveAt(index);
        return true;
    }
    ...
    bool IDictionary<Type, TObject>.ContainsKey(Type key) => ContainsKey(key); // { throw new NotImplementedException(); }

    // bool IReadOnlyDictionary<Type, object>.TryGetValue(Type key, out object value) { throw new NotImplementedException(); }
    ...
    // bool IReadOnlyDictionary<Type, object>.ContainsKey(Type key) { throw new NotImplementedException(); }
    bool IDictionary<Type, TObject>.TryGetValue(Type key, out TObject value) => TryGetValue(key, out value); // { throw new NotImplementedException(); }
    ...
    // int IReadOnlyCollection<KeyValuePair<Type, object>>.Count => _count;
    ...
    public struct Enumerator : IEnumerator<KeyValuePair<Type, TObject>> {
        ...
        public bool MoveNext() {
            _index++;
            if (_index >= _table._count) return false;
